...
torus.position.set(-6, 0.4, 3);   // translation
...
var marblePuddleMaterial = new THREE.ShaderMaterial({
		uniforms: { 
           vcsLightPosition: {value: new THREE.Vector3(0.0,0.0,-1.0) },
	   matrixWorld: {value: new THREE.Matrix4()},
           uNegxTexture: {type: 't', value: negxTexture},
	...
	vertexShader: document.getElementById('myVertShader').textContent,
	fragmentShader: document.getElementById('pnoiseFragShaderCreative').textContent,
		side: THREE.DoubleSide
});

marblePuddleGeometry = new THREE.PlaneBufferGeometry(15, 7.5);
marblePuddle = new THREE.Mesh(marblePuddleGeometry, marblePuddleMaterial);
marblePuddle.position.x = 0;
marblePuddle.position.z = -3.5;
marblePuddle.position.y = -1.0;
marblePuddle.rotation.x = -Math.PI/2;
scene.add(marblePuddle);
